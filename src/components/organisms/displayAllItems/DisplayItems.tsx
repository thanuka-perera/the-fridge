...
export const DisplayItems: FC<DisplayItemsProps> = ({
  foodItems,
}: DisplayItemsProps) => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      setIsUpdating(true);
      await updateFood(updateItemData._id!, {
        title: formItemData.title,
        expiry: formItemData.expiry,
      });
      router.refresh();
      ...
      {items.length === 0 ? (
        <div className="w-full flex flex-col items-center justify-center gap-3 p-6 border  border-gray-300 rounded-lg bg-white">
          <div className="text-gray-500 text-center">
            No items in the fridge. Start by adding some!
          </div>
        </div>
      ) : (
